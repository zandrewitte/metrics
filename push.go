...
type PushOptions struct {
	ClearSetOnSuccess bool
}

// InitPushWithOpts sets up periodic push for globally registered metrics to the given pushURL with the given interval
...
// It is OK calling InitPush multiple times with different pushURL -
// in this case metrics are pushed to all the provided pushURL urls.
func InitPushWithOpts(pushURL string, interval time.Duration, extraLabels string, pushProcessMetrics bool, opts ...PushOptions) error {
	writeMetrics := func(w io.Writer) {
		WritePrometheus(w, pushProcessMetrics)
	}
...
func InitPushExt(pushURL string, interval time.Duration, extraLabels string, writeMetrics func(w io.Writer), opts ...PushOptions) error {
	if interval <= 0 {
	...
	go func() {
		...
		for range ticker.C {
			...
			// success, check if we have the option to clear the default set
			for _, opt := range opts {
				if opt.ClearSetOnSuccess {
					defaultSet.UnregisterAllMetrics()
					println("unregistered all metrics, metric count: ", len(defaultSet.ListMetricNames()))
				}
			}
		}
	}()
